	...
	while (!quit) {
        ...
        if (acc_ticks >= time_step) {
            ...
            if (game_list_len > 0) {
                ...
                if (current_game > 0) {
                    SDL_Rect arrow_left_rect = {10, game_name_bg_pos.y + 20 - arrow_left->h / 2};
                    SDL_BlitSurface(arrow_left, NULL, screen, &arrow_left_rect);
                }

                if (current_game < game_list_len - 1) {
                    SDL_Rect arrow_right_rect = {630 - arrow_right->w, game_name_bg_pos.y + 20 - arrow_right->h / 2};
                ...
                SDL_Rect game_name_rect = {320 - game_name->w / 2, game_name_bg_pos.y + 20 - game_name->h / 2};
